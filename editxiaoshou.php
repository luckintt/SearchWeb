...
<center>
    ...
    //3.获取要修改的销售记录
    $sql="select  *  from  销售  where  商店名='{$_GET['商店名']}'  and  地址='{$_GET['地址']}'  and  商品名='{$_GET['商品名']}'";
    $result=mysqli_query($link,$sql);

    //4.判断是否获取到要编辑的销售记录
    if($result &&  mysqli_num_rows($result)>0){
        $shop=mysqli_fetch_assoc($result);  //解析出要修改的销售记录
    }else{
        die("没有找到要修改的销售记录！");
    }
    ...
    ?>
    <h3>编辑销售记录</h3>
    <form action="action.php?action=updatexiaoshou" enctype="multipart/form-data" method="post" >
        ...
        <table border="0"  width="310">
            ...
            <tr>
                ...
                <td><input type="text"  name="商品名" value="<?php  echo $shop['商品名']; ?>"/></td>
            </tr>
            <tr>
                <td align="right" valign="top">销售价格：</td>
                <td><input type="text"  name="销售价格" value="<?php  echo $shop['销售价格']; ?>"/></td>
            </tr>
            <tr>
