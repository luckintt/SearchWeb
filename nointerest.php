...
<div class="body">
    <form  method="post">
        ...
        <button class="searching" type="button" id="btn_search" onclick="check();">反选</button>
    </form>
</div>
<!--
    描述：返回数据的区域    开始
...
if( $商标==NULL  ||  empty( $商标)){
    header("location:nointerest.php");
}else{
...
//                                                (商品.商标 like '%{$商标}%'  or  词条.商品名 like '%{$商标}%');");
    ...
    $sql="select 词条.用户名,词条.商品图片,词条.商店名,词条.地址,词条.商品名,词条.销售价格,商家.印象标签,商家.口碑,词条.词条创建时间  from  词条,商品,商家
                                      where  词条.商店名=商家.商店名  and  词条.地址=商家.地址  and  商品.商品名=词条.商品名  and
                                                (商品.商标 not like '%{$商标}%'  and  词条.商品名 not like '%{$商标}%' and  商品.类型 not like '%{$商标}%') order by 口碑  DESC;";
