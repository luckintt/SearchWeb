...
<div class="body">
    <form  method="post">
        <input class="search" type="search" name="search2" id="search2" value="<?php  if(!empty( $_POST['search'])){echo $_POST['search'];} else {echo $_POST['search2'];} ?>" placeholder="输入您感兴趣的东西" />
        <button class="searching" type="button" id="btn_search" onclick="check();">搜索</button>
    </form>
</div>
<!--
    描述：返回数据的区域    开始
...
//查询商标是否存在
@$商标 = $_POST['search']?$_POST['search']:$_POST['search2'];
if( $商标==NULL  ||  empty( $商标)){
    header("location:result.php");
}else{
//    $result=mysqli_query($conn,"select  词条.商品图片,词条.商店名,词条.地址,词条.商品名,词条.销售价格,商家.印象标签,商家.口碑,词条.词条创建时间  from  词条,商品,商家
//                                            where  词条.商店名=商家.商店名  and  词条.地址=商家.地址  and  商品.商品名=词条.商品名  and
//                                                (商品.商标 like '%{$商标}%'  or  词条.商品名 like '%{$商标}%');");
    $result=mysqli_query($conn,"select 词条.用户名,词条.商品图片,词条.商店名,词条.地址,词条.商品名,词条.销售价格,商家.印象标签,商家.口碑,词条.词条创建时间  from  词条,商品,商家
                                            where  词条.商店名=商家.商店名  and  词条.地址=商家.地址  and  商品.商品名=词条.商品名  and
                                                (商品.商标 like '%{$商标}%'  or  词条.商品名 like '%{$商标}%' or 商品.类型 like '%{$商标}%') order by 口碑  DESC;");
...
//        echo '<li class="tar">印象：<a class="effect">' . "{$印象标签[$i]}" . "</a></li>";
